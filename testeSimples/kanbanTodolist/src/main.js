...
// 	if(idInputTask.value.trim() === '') return
...
// Função para definir o HTML das tarefas
function setHtml() {
    document.getElementById('add-task').innerHTML = '';
    ...
    // Renderiza as informações do banco de dados no HTML
    db.forEach((list, index) => {
        addTask(list.inputDB, list.checkedDB, index);
    });
}

// Função para criar o HTML das tarefas
function addTask(text, checkedDB, index) {
    const div = document.createElement('div');
    div.classList.add('todos');
    div.innerHTML = `
        <article class="tasks" draggable="true">
            ...
            <div class="tasks_text">
                <p class="tasks-p" data-p="${index}">${text}</p>
            </div>
            <div class="trash" data-id=${index} onclick="deleteTask(${index})">
               <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
 					<path d="M14.2021 9.98548H12.8716V15.5073H14.2021V9.98548Z" fill="#808080"/>
    ...
    // Define estilo para o texto checado
    if (checkedDB) {
        div.querySelector('.tasks-p').style.textDecoration = 'line-through';
    }

    // Adiciona eventos de drag and drop
    const taskElement = div.querySelector('.tasks');
    ...
    taskElement.addEventListener('dragover', (e) => {
        e.preventDefault();
        const draggingElement = document.querySelector('.tasks.dragging');
        const currentElement = e.currentTarget;
		
        if (draggingElement !== currentElement) {
			const allTasks = document.querySelectorAll('.tasks');
            const indexCurrent = [...allTasks].indexOf(currentElement);
            if (indexCurrent < draggedItemIndex) {
...
// Atualiza o banco de dados após arrastar
function updateOrder() {
    const tasks = document.querySelectorAll('.tasks');
    db = [...tasks].map((task, index) => {
        const checkbox = task.querySelector('.checked_input');
        return {
            inputDB: task.querySelector('.tasks-p').innerText,
            checkedDB: checkbox.checked ? 'checked' : ''
        };
    });
    localStorage.setItem('todolist', JSON.stringify(db));
}

// Atualiza o HTML ao final do drag and drop
document.querySelectorAll('.todos').forEach((itens) => {
    itens.addEventListener('drop', (e) => {
        updateOrder();
        setHtml();
...
function checkboxUp(checkbox, index) {
    db[index].checkedDB = checkbox.checked ? 'checked' : '';
    localStorage.setItem('todolist', JSON.stringify(db));
    setHtml();
}
...
function addNewTask() {
    const taskInput = document.getElementById('idInputTask');
    if (taskInput.value.trim()) {
        db.push({ inputDB: taskInput.value, checkedDB: '' });
        localStorage.setItem('todolist', JSON.stringify(db));
        taskInput.value = '';
        setHtml();
    }	
...
// Função para contar as tarefas completadas
function countCompletedTasks() {
    return db.filter(task => task.checkedDB === 'checked').length;
}